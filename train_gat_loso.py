...
# Subject-level aggregation used inside the model. Mean for now — later this can be
# swapped for e.g. "mean_std" or "attention" without changing the rest of the model.
AGGREGATION = "mean"
...
def run_loso():
    ...
    for fold_index, test_subject in enumerate(subjects, start=1):
        ...
        # Validation subjects must be separate from both the training set used for
        # optimisation and the held-out test subject. For now we take the final 10%
        # of training subjects as validation subjects; this will be made deterministic.
        validation_count = max(2, int(round(len(training_subjects) * 0.10)))
        validation_subjects = training_subjects[-validation_count:]
        actual_training_subjects = training_subjects[:-validation_count]
