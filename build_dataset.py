...
# Extract spectral features from EEG using Welch's method
...
def calculate_spectral_features(data):
    ...
    relative = absolute / (total_power[:, None] + 1e-12)
    ...
    features = [absolute, relative, entropy[:, None]]
...
def calculate_pearson_graph(data):
    # Each channel keeps its TOP_K strongest correlations as directed edges
    ...
    for source in range(N_CHANNELS):
        row = correlation[source].copy()
        row[source] = -np.inf
        neighbours = np.argpartition(row, -TOP_K)[-TOP_K:]
        for target in neighbours:
            sources.append(source)
            targets.append(target)
            weights.append(correlation[source, target])
...
def main():
    ...
    print(f"Connectivity:    Pearson")
    print(f"Top-K:           {TOP_K}")
    print("Normalisation:   NONE — performed per LOSO fold")
    print()
    if not files:
    ...
    metadata = {
        "configuration": {
            ...
            "normalisation": (
                "None during dataset building. "
                "StandardScaler must be fit "
                "on training subjects within "
                "each LOSO fold."
            )
        },
        "subjects": {
